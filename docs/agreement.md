...
# SPlayer Together 用户协议与免责声明

**版本：v1.3**<br>
**生效日期：2026 年 5 月 29 日**<br>
**最近更新：2026 年 8 月 20 日**

欢迎使用 SPlayer Together（以下简称“本软件”）。本软件是一款免费、开源的跨平台桌面音乐播放器。在您下载、安装、复制、运行或以任何方式使用本软件之前，请务必认真阅读并充分理解本协议的全部内容，**尤其是以加粗或其他显著方式标识的免除或限制责任、争议解决与适用法律等条款**。
...
> **独立项目声明：** SPlayer Together 是由 `Aruvelut-123` 独立维护和发布的 SPlayer Next 分支，不是 SPlayer-Dev、SPlayer Next 或原始 SPlayer 项目的官方版本，与其不存在隶属、授权、维护或背书关系。上游项目名称、版权和许可证声明仍归其各自权利人所有。

为便于理解，本协议中下列术语的含义如下：

- **“开发者”**：指 SPlayer Together 的独立维护者 `Aruvelut-123` 及本分支的开源贡献者；上游 `SPlayer-Dev` 项目及其贡献者不属于本项目开发者，除非明确说明。
...
- 不得对本软件附加任何与 AGPL-3.0 相抵触的额外限制。
  ...
  3.4 本分支源代码和发布地址为：<https://github.com/Aruvelut-123/SPlayer-Together>。本分支基于上游项目 <https://github.com/SPlayer-Dev/SPlayer-Next>，但不是上游项目的官方版本。
...
7.2 “SPlayer”“SPlayer Together”等名称、标识及相关视觉设计的权益归开发者所有，未经许可，您不得用于可能造成混淆、误导或商业目的的场景。
